...
Java_com_leonel_nuklear_nuklearActivity_setAssetsNativeManager(JNIEnv *env, jobject thiz, jobject asset_manager)
{
...
unsigned char * getAssetFile(const char *filename, int *x, int *y, int *n)
{
    ...
    if (fileAsset)
    {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, TAG, "Asset file size: %d\n", asset_length);
