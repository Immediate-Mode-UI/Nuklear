...
bool create_instance(struct vulkan_demo *demo) {
    ...
    const char*const* found_extensions =
        SDL_Vulkan_GetInstanceExtensions(&sdl_extension_count);
    if (found_extensions == NULL)
    ...
    memcpy(enabled_extensions, found_extensions, sdl_extension_count*sizeof(char *));
...
VkExtent2D choose_swap_extent(struct vulkan_demo *demo,
    ...
    if (capabilities->currentExtent.width != 0xFFFFFFFF) {
        ...
        /* not window size! */
        SDL_GetWindowSizeInPixels(demo->win, &width, &height);
...
bool create_graphics_pipeline(struct vulkan_demo *demo) {
    bool ret = false;
    uint32_t *vert_shader_code = NULL;
    uint32_t *frag_shader_code = NULL;
    ...
    fseek(fp, 0, 0);
    read_result = fread(frag_shader_code, 4, file_len, fp);
    fclose(fp);
    if (read_result != file_len/4) {
