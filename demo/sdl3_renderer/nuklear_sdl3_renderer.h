...
nk_sdl_render(struct nk_context* ctx, enum nk_anti_aliasing AA)
{
    ...
    if (active != sdl->edit_was_active)
    {
        ...
        if (!sdl_active && !sdl->edit_was_active && active)
...
nk_sdl_handle_event(struct nk_context* ctx, SDL_Event *evt)
{
    ...
    /* SDL_ConvertEventToRenderCoordinates may mutate 'evt' so we use its copy from now on */
    NK_MEMCPY(&evtcpy, evt, sizeof(evtcpy));

    if (!SDL_ConvertEventToRenderCoordinates(sdl->renderer, &evtcpy)) {
        NK_ASSERT(!"SDL_ConvertEventToRenderCoordinates should never fail here!");
    }
    ...
    switch(evtcpy.type)
    {
        ...
        case SDL_EVENT_KEY_DOWN:
            {
                int down = evtcpy.type == SDL_EVENT_KEY_DOWN;
                int ctrl_down = evtcpy.key.mod & (SDL_KMOD_LCTRL | SDL_KMOD_RCTRL);

                /* In 99% of the time, you want to use scancodes, not real key codes,
                 * see: https://wiki.libsdl.org/SDL3/BestKeyboardPractices */
                switch(evtcpy.key.scancode)
                {
        ...
        case SDL_EVENT_MOUSE_BUTTON_DOWN:
            {
                const int x = evtcpy.button.x, y = evtcpy.button.y;
                const int down = evtcpy.button.down;
                const double dt = (double)(evtcpy.button.timestamp - sdl->last_left_click) / 1000000000.0;
                switch(evtcpy.button.button)
                {
                    case SDL_BUTTON_LEFT:
                        nk_input_button(ctx, NK_BUTTON_LEFT, x, y, down);
                        nk_input_button(ctx, NK_BUTTON_DOUBLE, x, y,
                                down && dt > NK_SDL_DOUBLE_CLICK_LO && dt < NK_SDL_DOUBLE_CLICK_HI);
                        sdl->last_left_click = evtcpy.button.timestamp;
                        break;
        ...
        case SDL_EVENT_MOUSE_MOTION:
            ctx->input.mouse.pos.x = evtcpy.motion.x;
            ctx->input.mouse.pos.y = evtcpy.motion.y;
        ...
        case SDL_EVENT_TEXT_INPUT:
            {
                nk_glyph glyph;
                nk_size len;
                NK_ASSERT(evtcpy.text.text);
                len = SDL_strlen(evtcpy.text.text);
                NK_ASSERT(len <= NK_UTF_SIZE);
                NK_MEMCPY(glyph, evtcpy.text.text, len);
                nk_input_glyph(ctx, glyph);
            }
            return 1;

        case SDL_EVENT_MOUSE_WHEEL:
            nk_input_scroll(ctx, nk_vec2(evtcpy.wheel.x, evtcpy.wheel.y));
            return 1;
    }
    return 0;
}
