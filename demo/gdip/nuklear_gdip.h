...
nk_gdip_handle_event(HWND wnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_LBUTTONDOWN:
        nk_input_button(&gdip.ctx, NK_BUTTON_LEFT, (float)LOWORD(lparam), (float)HIWORD(lparam), 1);
        SetCapture(wnd);
        return 1;

    case WM_LBUTTONUP:
        nk_input_button(&gdip.ctx, NK_BUTTON_DOUBLE, (float)LOWORD(lparam), (float)HIWORD(lparam), 0);
        nk_input_button(&gdip.ctx, NK_BUTTON_LEFT, (float)LOWORD(lparam), (float)HIWORD(lparam), 0);
        ReleaseCapture();
        return 1;

    case WM_RBUTTONDOWN:
        nk_input_button(&gdip.ctx, NK_BUTTON_RIGHT, (float)LOWORD(lparam), (float)HIWORD(lparam), 1);
        SetCapture(wnd);
        return 1;

    case WM_RBUTTONUP:
        nk_input_button(&gdip.ctx, NK_BUTTON_RIGHT, (float)LOWORD(lparam), (float)HIWORD(lparam), 0);
        ReleaseCapture();
        return 1;

    case WM_MBUTTONDOWN:
        nk_input_button(&gdip.ctx, NK_BUTTON_MIDDLE, (float)LOWORD(lparam), (float)HIWORD(lparam), 1);
        SetCapture(wnd);
        return 1;

    case WM_MBUTTONUP:
        nk_input_button(&gdip.ctx, NK_BUTTON_MIDDLE, (float)LOWORD(lparam), (float)HIWORD(lparam), 0);
        ReleaseCapture();
        return 1;
    ...
    case WM_MOUSEMOVE:
        nk_input_motion(&gdip.ctx, (float)LOWORD(lparam), (float)HIWORD(lparam));
        return 1;

    case WM_LBUTTONDBLCLK:
        nk_input_button(&gdip.ctx, NK_BUTTON_DOUBLE, (float)LOWORD(lparam), (float)HIWORD(lparam), 1);
        return 1;
    }
