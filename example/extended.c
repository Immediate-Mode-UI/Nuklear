...
grid_demo(struct nk_context *ctx, struct media *media)
{
        ...
        NK_WINDOW_NO_SCROLLBAR))
    {
        ...
        if (nk_combo_begin_label(ctx, items[selected_item], nk_vec2(nk_widget_width(ctx), 200), NK_DROP_AUTO)) {
...
basic_demo(struct nk_context *ctx, struct media *media)
{
    ...
    ui_widget(ctx, media, 40);
    if (nk_combo_begin_image_label(ctx, items[selected_icon], media->images[selected_icon], nk_vec2(nk_widget_width(ctx), 200), NK_DROP_AUTO)) {
        nk_layout_row_dynamic(ctx, 35, 1);
        for (i = 0; i < 3; ++i)
